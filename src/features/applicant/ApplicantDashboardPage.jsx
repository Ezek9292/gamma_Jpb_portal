...
export function ApplicantDashboardPage() {
  ...
  const mine = applications.filter((item) => item.applicantId === currentUser.id).map((application) => ({ ...application, job: application.job || jobs.find((job) => job.id === application.jobId) })).filter((item) => item.job);
  return (
    <div className="page-shell py-12 sm:py-16">
      <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="eyebrow">Applicant dashboard</p><h1 className="page-title mt-3">Welcome back, {currentUser.name.split(' ')[0]}.</h1><p className="mt-3 text-slate">Review and track all your job applications in one place.</p></div><Link to="/jobs" className="btn-primary">Browse more jobs <ArrowRight size={17} /></Link></div>
