...
export function ProfilePage() {
  ...
  const [form, setForm] = useState(currentProfile || { fullName: currentUser.name || '', phone: '', bio: '', skills: [], experience: '', education: '', resumeName: '' });
  ...
  const submit = (event) => { event.preventDefault(); saveProfile(form); setSaved(true); if (location.state?.from) navigate(location.state.from, { replace: true }); };
  ...
  return (
    <div className="page-shell py-12 sm:py-16">
      ...
      <form onSubmit={submit} className="mt-9 max-w-3xl space-y-7">
        ...
        <section className="panel grid gap-5"><h2 className="font-display text-xl font-bold">Background</h2><label><span className="label">Experience</span><textarea className="field min-h-28" name="experience" value={form.experience} onChange={update} placeholder="Role, company, dates, and your most useful impact" /></label><label><span className="label">Education</span><textarea className="field min-h-24" name="education" value={form.education} onChange={update} placeholder="Qualification, institution, year" /></label><label><span className="label">Resume</span><span className="flex cursor-pointer items-center gap-4 rounded-xl border border-dashed border-ink/25 p-5 hover:border-amber"><span className="grid h-10 w-10 place-items-center rounded-lg bg-amber/15 text-coral"><FileUp size={20} /></span><span className="text-sm"><strong className="block font-display">{form.resumeName || 'Choose a PDF or DOC file'}</strong><span className="text-xs text-slate">File name is stored for this frontend demo</span></span><input className="sr-only" type="file" accept=".pdf,.doc,.docx" onChange={(event) => setForm((value) => ({ ...value, resumeName: event.target.files?.[0]?.name || value.resumeName }))} /></span></label></section>
        <div className="flex items-center gap-4"><button className="btn-primary" type="submit">Save profile <Check size={17} /></button>{saved && <span className="text-sm font-semibold text-forest">Profile saved</span>}</div>
      </form>
    </div>
  );
}
