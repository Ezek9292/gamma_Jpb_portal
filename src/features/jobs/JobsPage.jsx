...
export function JobsPage() {
  ...
  return (
    <div className="page-shell py-12 sm:py-16">
      <div className="max-w-3xl"><p className="eyebrow">Departure board</p><h1 className="page-title mt-3">Find where you’re going next.</h1><p className="mt-4 text-lg text-slate">Search open roles from teams building things that matter.</p></div>
      ...
      {filteredJobs.length ? <div className="grid gap-5">{filteredJobs.map((job) => <JobCard key={job.id} job={job} />)}</div> : <div className="rounded-2xl border border-dashed border-ink/25 bg-paperCard px-6 py-16 text-center"><SlidersHorizontal className="mx-auto text-amber" size={35} /><h2 className="mt-5 font-display text-2xl font-bold">No departures found</h2><p className="mx-auto mt-2 max-w-md text-slate">Try a broader search or clear your filters to see all available roles.</p><button onClick={clearFilters} className="btn-primary mt-6">Clear all filters</button></div>}
    </div>
  );
}
