import { ArrowRight, BriefcaseBusiness, Search, Sparkles, TicketCheck } from 'lucide-react';
    ...
    <>
      <section className="overflow-hidden bg-ink pb-20 pt-16 text-white sm:pb-28 sm:pt-24">
        <div className="page-shell relative">
          ...
          <div className="relative max-w-4xl">
            <p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-amber">Start your next chapter</p>
            <h1 className="mt-5 font-display text-5xl font-bold leading-[0.98] tracking-[-0.055em] sm:text-7xl lg:text-[5.5rem]">Work worth<br /><span className="text-amber">showing up for.</span></h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-white/70 sm:text-xl">Discover ambitious teams and meaningful roles across Africa and beyond. No noise—just opportunities with somewhere to go.</p>
          </div>
      ...
      <section className="page-shell py-20 sm:py-24">
        <div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div><p className="eyebrow">Featured Roles</p><h2 className="mt-3 font-display text-3xl font-bold tracking-[-0.04em] sm:text-4xl">Ready when you are.</h2></div>
          <Link to="/jobs" className="inline-flex items-center gap-2 font-display text-sm font-bold text-coral hover:gap-3">View every role <ArrowRight size={17} /></Link>
        </div>
      ...
      <section className="border-y border-ink/10 bg-white/45 py-20">
        <div className="page-shell">
          <div className="max-w-xl"><p className="eyebrow">A simpler route</p><h2 className="mt-3 font-display text-3xl font-bold tracking-[-0.04em] sm:text-4xl">From browsing to boarding in three steps.</h2></div>
          <div className="mt-12 grid gap-8 md:grid-cols-3">
            {[
              [Search, '01', 'Find your fit', 'Search by role, company, location, or the kind of work you want to do.'],
              [Sparkles, '02', 'Tell your story', 'Build one focused profile that puts your skills and experience up front.'],
              [TicketCheck, '03', 'Make your move', 'Apply in a click and keep every opportunity organized in one place.'],
      ...
      <section className="page-shell py-20 sm:py-24">
        <div className="flex flex-col items-start justify-between gap-8 rounded-3xl bg-amber p-8 sm:flex-row sm:items-center sm:p-12">
          <div className="max-w-2xl"><BriefcaseBusiness size={30} /><h2 className="mt-5 font-display text-3xl font-bold tracking-[-0.04em] sm:text-4xl">Your next role won’t find itself.</h2><p className="mt-3 text-ink/70">Create a profile and be ready when the right opportunity lands.</p></div>
