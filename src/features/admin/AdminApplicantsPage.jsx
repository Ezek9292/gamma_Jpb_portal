...
function DocumentButton({ file, fallbackName, label }) {
  ...
  return <button type="button" onClick={openDocument} disabled={!file} title={name || `${label} not provided`} className="btn-secondary !min-h-9 !max-w-full !px-4 !py-1.5"><FileText size={15} /><span className="max-w-52 truncate">{name || `No ${label.toLowerCase()}`}</span></button>;
}
