...
export function NewJobPage() {
  ...
  const submit = async (event) => {
    ...
    setSubmitting(true);
    try {
      await addJob({ ...job, requirements: requirementsText.split('\n').map((item) => item.trim()).filter(Boolean), featured: false });
  ...
  return (
    <div className="page-shell py-12 sm:py-16">
      ...
      <div className="mt-7 max-w-3xl"><p className="eyebrow">New job listing</p><h1 className="page-title mt-3">Post a new job.</h1><p className="mt-4 text-slate">Provide applicants with clear information about the role and its requirements.</p></div>
      <form onSubmit={submit} className="mt-9 max-w-3xl space-y-7">
        {error && <p className="rounded-lg bg-coral/10 p-3 text-sm text-coral" role="alert">{error}</p>}
        ...
        <div className="flex flex-wrap gap-3"><button type="submit" className="btn-primary" disabled={submitting}><Plus size={18} />{submitting ? 'Publishing…' : 'Publish job'}</button><Link to="/admin/dashboard" className="btn-secondary">Cancel</Link></div>
      </form>
    </div>
  );
}
